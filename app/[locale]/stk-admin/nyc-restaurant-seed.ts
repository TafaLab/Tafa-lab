...
type AuditedNycRestaurantLead={id:string;created_at:string;name:string;contact:string;company:string;country:string;city:string;project_type:string;category:string;message:string|null;locale:"ru";source_path:string;status:"new"|"draft";admin_notes:null;seedProfitability:"high"|"medium"|"low"};
const cleanRestaurantLead=(lead:NycRestaurantLead):AuditedNycRestaurantLead=>{const unverified=/^Телефон:\s*Restaurants\b/i.test(lead.contact);return {...lead,contact:unverified?"Контакт не найден — требуется проверка":String(lead.contact),message:unverified?"Контакты, адрес и официальный сайт требуют проверки перед обращением.":lead.message,seedProfitability:unverified?"medium":lead.seedProfitability as "high"|"medium"|"low",status:unverified?"draft":"new"} as AuditedNycRestaurantLead};
