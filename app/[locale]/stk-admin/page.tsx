...
export default function StkAdminPage() {
  ...
  const [newLead,setNewLead]=useState({name:"",phone:"",instagram:"",email:"",company:"",project_type:"",message:"",reminder_at:""});
  ...
  function createCrmLead(e:FormEvent<HTMLFormElement>){
    ...
    const lead:Lead={id,created_at:new Date().toISOString(),name:newLead.name.trim(),contact,company:newLead.company.trim()||null,project_type:newLead.project_type.trim()||null,message:newLead.message.trim()||null,locale:"ru",source_path:"Добавлено вручную",status:"new",admin_notes:null};
    const manual=(()=>{try{return JSON.parse(localStorage.getItem("stk-admin-manual-crm")||"[]") as Lead[]}catch{return []}})();
    manual.unshift(lead);
    ...
    setNewLead({name:"",phone:"",instagram:"",email:"",company:"",project_type:"",message:"",reminder_at:""});setAdding(false);setSection("crm");setSelectedId(id);openLead(lead);
  }
