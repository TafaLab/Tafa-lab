...
const text = {
  ru: {
    ...
    filters:{all:"Все",new:"Новые",draft:"Черновики",contacted:"Связались",in_progress:"В работе",won:"Успешно",lost:"Отказ",dead:"Мёртвые",not_profitable:"Нерентабельные"},
    statuses:{new:"Новая",draft:"Черновик",contacted:"Связались",in_progress:"В работе",won:"Успешно",lost:"Отказ",dead:"Мёртвая",not_profitable:"Нерентабельная"},
  ...
  en: {
    ...
    filters:{all:"All",new:"New",draft:"Drafts",contacted:"Contacted",in_progress:"In progress",won:"Won",lost:"Lost",dead:"Dead",not_profitable:"Not profitable"},
    statuses:{new:"New",draft:"Draft",contacted:"Contacted",in_progress:"In progress",won:"Won",lost:"Lost",dead:"Dead",not_profitable:"Not profitable"},
...
const statusStyles:Record<LeadStatus,string>={
  ...
  in_progress:"bg-[#eee8f6] text-[#4e3c62]",won:"bg-[#e6f1e8] text-[#35503a]",lost:"bg-[#f5e6e5] text-[#663f3c]",dead:"bg-[#e5e5e5] text-[#555555]",not_profitable:"bg-[#fff0d9] text-[#805b20]",
};
...
function leadCategories(lead:Lead,meta?:CrmMeta){const saved=categoryValues(meta?.category);if(saved.length)return saved;if(!isSeededBakery(lead))return [];return lead.name.trim().toLowerCase()==="fika"?["Кондитерская","Ресторан"]:["Кондитерская"]}
...
function interactionTimeline(meta?:CrmMeta){
  ...
  const legacy=(meta?.history||[]).filter(item=>!seen.has(`${item.created_at}|${item.text}`)).map((item,index):InteractionEntry=>({id:`legacy-note-${item.created_at}-${index}`,channel:"note",text:item.text,created_at:item.created_at}));
  return [...interactions,...legacy].sort((a,b)=>+new Date(b.created_at)-+new Date(a.created_at));
}
function interactionLabel(channel:InteractionChannel,locale:"ru"|"en"){
  const labels=locale==="ru"?{whatsapp:"WhatsApp",email:"Email",instagram:"Instagram",note:"Заметка",status:"Изменение статуса"}:{whatsapp:"WhatsApp",email:"Email",instagram:"Instagram",note:"Note",status:"Status change"};
...
export default function StkAdminPage(){
  ...
  const analytics=useMemo(()=>{
    ...
    const categoryCounts=Object.entries(crmLeads.reduce<Record<string,number>>((acc,lead)=>{const values=leadCategories(lead,crmMeta[lead.id]);(values.length?values:["Не указано"]).forEach(value=>acc[value]=(acc[value]||0)+1);return acc},{})).sort((a,b)=>b[1]-a[1]).slice(0,10);
    return {total,contacted,won,overdue,conversion:total?Math.round(won/total*100):0,sources:group(x=>crmMeta[x.id]?.source||x.source_path||""),categories:categoryCounts};
  },[crmLeads,crmMeta]);
  ...
  function exportExcel(){
    ...
    const rows=crmLeads.map(lead=>{const fields=contactFields(lead.contact),meta=crmMeta[lead.id];return [lead.name,fields.phone,fields.instagram,fields.email,lead.company||"",lead.city||"",lead.project_type||"",leadCategories(lead,meta).join(", "),(meta?.tags||[]).join(", "),meta?.source||lead.source_path||"",lead.status,reminderDate(meta),lead.message||""]});
  ...
  const selected=leads.find(x=>x.id===selectedId)||null,filterKeys:LeadFilter[]=["all","new","draft","contacted","in_progress","won","lost","dead","not_profitable"];
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col md:flex-row">
      ...
      <section className="min-w-0 flex-1 px-5 py-8 md:px-8 md:py-10">
        ...
        {adding&&section==="crm"&&<form onSubmit={createCrmLead} className="mt-6 rounded-[28px] border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xl">{locale==="ru"?"Новая запись CRM":"New CRM record"}</h2><button type="button" onClick={()=>setAdding(false)} className="text-2xl">×</button></div><div className="mt-5 grid gap-3 md:grid-cols-2">
          ...
          <input value={newLead.company} onChange={e=>setNewLead({...newLead,company:e.target.value})} placeholder={locale==="ru"?"Компания":"Company"} className="rounded-2xl border border-black/10 px-4 py-3"/><input value={newLead.city} onChange={e=>setNewLead({...newLead,city:e.target.value})} placeholder={locale==="ru"?"Город":"City"} className="rounded-2xl border border-black/10 px-4 py-3"/><input value={newLead.project_type} onChange={e=>setNewLead({...newLead,project_type:e.target.value})} placeholder={locale==="ru"?"Что предложить":"Project type"} className="rounded-2xl border border-black/10 px-4 py-3"/><div className="rounded-2xl border border-black/10 p-3 md:col-span-2"><div className="mb-2 text-xs uppercase text-black/40">{locale==="ru"?"Категории — можно выбрать несколько":"Categories — select multiple"}</div><div className="flex flex-wrap gap-2">{CRM_CATEGORY_OPTIONS.map(category=><button type="button" key={category} onClick={()=>setNewLead({...newLead,category:toggleCategory(newLead.category,category)})} className={`rounded-full border px-3 py-2 text-xs ${categoryValues(newLead.category).includes(category)?"border-[#211a17] bg-[#211a17] text-white":"border-black/10 bg-white"}`}>{category}</button>)}</div></div>
        ...
        {section==="kanban"&&<div className="mt-8 overflow-x-auto"><div className="grid min-w-[1680px] grid-cols-8 gap-3">{(["new","draft","contacted","in_progress","won","lost","dead","not_profitable"] as LeadStatus[]).map(status=><div key={status} onDragOver={e=>e.preventDefault()} onDrop={e=>void changeStatus(e.dataTransfer.getData("text/plain"),status)} className="min-h-[500px] rounded-[24px] border border-black/10 bg-white/60 p-3"><div className="flex justify-between px-2 py-2"><b className="text-sm">{t.statuses[status]}</b><span className="text-xs">{crmLeads.filter(x=>x.status===status).length}</span></div><div className="mt-2 space-y-3">{crmLeads.filter(x=>x.status===status).map(lead=><button key={lead.id} draggable onDragStart={e=>e.dataTransfer.setData("text/plain",lead.id)} onClick={()=>fillDraft(lead)} className={`block w-full rounded-2xl border p-3 text-left text-sm shadow-sm ${reminderTone(crmMeta[lead.id])||"border-black/10 bg-white"}`}><b>{lead.name}</b><div className="mt-2 text-xs text-black/45">{leadCategories(lead,crmMeta[lead.id]).join(", ")||lead.project_type||"—"}</div>{crmMeta[lead.id]?.reminder_at&&<div className="mt-2 text-xs">◷ {crmMeta[lead.id]?.reminder_at} {crmMeta[lead.id]?.reminder_time||""}</div>}</button>)}</div></div>)}</div></div>}
        ...
        {selected&&<><button type="button" aria-label={t.close} onClick={()=>setSelectedId(null)} className="fixed inset-0 z-30 bg-black/35 backdrop-blur-[2px]"/><aside className="fixed inset-y-0 right-0 z-40 w-full max-w-[520px] overflow-y-auto border-l border-black/10 bg-[#f5f1ec] p-4 shadow-2xl md:p-6"><div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm"><div className="flex justify-between gap-4"><div><p className="text-xs uppercase text-black/35">{t.lead}</p><h2 className="mt-2 text-2xl">{selected.name}</h2></div><button onClick={()=>setSelectedId(null)} className="h-9 w-9 rounded-full border border-black/10">×</button></div><div className="mt-6 space-y-5">
          <label className="block text-xs text-black/40">{t.status}<select value={draftStatus} onChange={e=>setDraftStatus(e.target.value as LeadStatus)} className="mt-2 w-full rounded-2xl border border-black/10 bg-[#faf8f6] px-4 py-3">{(["new","draft","contacted","in_progress","won","lost","dead","not_profitable"] as LeadStatus[]).map(status=><option key={status} value={status}>{t.statuses[status]}</option>)}</select></label>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs text-black/40">{locale==="ru"?"Компания":"Company"}<input value={draftCompany} onChange={e=>setDraftCompany(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 bg-[#faf8f6] px-4 py-3"/></label><label className="text-xs text-black/40">{locale==="ru"?"Город":"City"}<input value={draftCity} onChange={e=>setDraftCity(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 bg-[#faf8f6] px-4 py-3"/></label><div className="text-xs text-black/40 sm:col-span-2"><div>{locale==="ru"?"Категории — можно выбрать несколько":"Categories — select multiple"}</div><div className="mt-2 flex flex-wrap gap-2 rounded-2xl border border-black/10 bg-[#faf8f6] p-3">{categoryOptions.map(category=><button type="button" key={category} onClick={()=>setDraftCategory(toggleCategory(draftCategory,category))} className={`rounded-full border px-3 py-2 text-xs ${categoryValues(draftCategory).includes(category)?"border-[#211a17] bg-[#211a17] text-white":"border-black/10 bg-white text-black"}`}>{category}</button>)}</div></div><label className="text-xs text-black/40 sm:col-span-2">{locale==="ru"?"Источник":"Source"}<input value={draftSource} onChange={e=>setDraftSource(e.target.value)} className="mt-2 w-full rounded-2xl border border-black/10 bg-[#faf8f6] px-4 py-3"/></label></div>
          ...
          <div className="border-t border-black/10 pt-5"><div className="text-xs uppercase text-black/40">{locale==="ru"?"История взаимодействий":"Interaction history"}</div><div className="mt-3 space-y-3">{interactionTimeline(crmMeta[selected.id]).map(item=><div key={item.id} className="rounded-2xl bg-[#faf8f6] p-3"><div className="flex justify-between gap-3 text-xs text-black/40"><span>{interactionLabel(item.channel,locale)}</span><span>{new Date(item.created_at).toLocaleString(locale==="ru"?"ru-RU":"en-US")}</span></div><p className="mt-2 whitespace-pre-wrap text-sm">{item.text}</p></div>)}{interactionTimeline(crmMeta[selected.id]).length===0&&<p className="text-sm text-black/40">{locale==="ru"?"История пока пустая":"No interactions yet"}</p>}</div></div>
