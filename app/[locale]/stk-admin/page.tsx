  ...
  return <main className="min-h-screen bg-[#f5f1ec] text-[#211a17]">
    ...
    <div className="mx-auto flex max-w-[1600px] flex-col md:flex-row">
      ...
      <section className="min-w-0 flex-1 px-5 py-8 md:px-8 md:py-10">
        ...
        {section==="kanban"&&<div className="mt-8 overflow-x-auto"><div className="grid min-w-[1260px] grid-cols-6 gap-3">{(["new","draft","contacted","in_progress","won","lost","dead","not_profitable"] as LeadStatus[]).map(status=><div key={status} onDragOver={e=>e.preventDefault()} onDrop={e=>void changeStatus(e.dataTransfer.getData("text/plain"),status)} className="min-h-[500px] rounded-[24px] border border-black/10 bg-white/60 p-3"><div className="flex justify-between px-2 py-2"><b className="text-sm">{t.statuses[status]}</b><span className="text-xs">{crmLeads.filter(x=>x.status===status).length}</span></div><div className="mt-2 space-y-3">{crmLeads.filter(x=>x.status===status).map(lead=><button key={lead.id} draggable onDragStart={e=>e.dataTransfer.setData("text/plain",lead.id)} onClick={()=>fillDraft(lead)} className={`block w-full rounded-2xl border p-3 text-left text-sm shadow-sm ${reminderTone(crmMeta[lead.id])||"border-black/10 bg-white"}`}><b>{lead.name}</b><div className="mt-2 text-xs text-black/45">{leadCategories(lead,crmMeta[lead.id]).join(", ")||lead.project_type||"—"}</div>{crmMeta[lead.id]?.reminder_at&&<div className="mt-2 text-xs">◷ {crmMeta[lead.id]?.reminder_at} {crmMeta[lead.id]?.reminder_time||""}</div>}</button>)}</div></div>)}</div></div>}
