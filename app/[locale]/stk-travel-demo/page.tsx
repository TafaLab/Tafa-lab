...
export default async function Page({params}:{params:Promise<{locale:string}>}) {
  ...
  return <main className="min-h-screen bg-[#f4efe5] text-[#183a31]">
    ...
    <section className="bg-[#d9b957] px-5 py-20 text-[#183a31] md:px-10 md:py-28"><div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[.6fr_1.4fr]"><p className="text-xs font-semibold uppercase tracking-[.28em]">{locale==="ru"?"От местного автора":"From a local author"}</p><div><h2 className="text-5xl font-black tracking-[-.05em] md:text-7xl">{locale==="ru"?"Киото до восьми утра принадлежит тишине.":"Before eight, Kyoto belongs to quiet."}</h2><p className="mt-7 max-w-3xl text-lg leading-8 text-black/60">{locale==="ru"?"Юки живёт у северного края Императорского сада и составляет маршруты без очередей: утренний рынок, семейная мастерская бумаги и маленький бар, где меню меняется вместе с погодой. Наши гайды пишут люди, которые действительно живут на месте.":"Yuki lives near the north edge of the Imperial Garden and designs routes without queues: the morning market, a family paper workshop and a tiny bar whose menu changes with the weather. Our guides are written by people who actually live there."}</p><p className="mt-7 font-semibold">YUKI TANAKA · KYOTO</p></div></div></section>
