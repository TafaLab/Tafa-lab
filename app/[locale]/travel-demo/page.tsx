...
const copy = {
  ru: {
    ...
    cards: [["Амальфитанское побережье","Италия · 8 дней","от $3 900"],["Тихая роскошь Бали","Индонезия · 10 дней","от $2 800"],["Япония в сезон сакуры","Токио — Киото · 12 дней","от $5 200"],["Острова Греции","Киклады · 9 дней","от $3 400"],["Дикая Исландия","Рейкьявик — Вик · 7 дней","от $4 100"],["Сафари и океан","Кения — Занзибар · 11 дней","от $6 300"]],
    stylesEyebrow: "Путешествуйте по-своему", stylesTitle: "Какое путешествие подходит именно вам?",
    styles: [["Пляжи и острова","Уединённые виллы, прозрачная вода и неспешные дни у океана."],["Гастрономические маршруты","Авторские рестораны, винодельни, рынки и встречи с локальными шефами."],["Приключения и природа","Сафари, вулканы, трекинг и места, где пейзаж становится главным событием."],["Культура и города","Архитектура, искусство и частные экскурсии без стандартных туристических сценариев."],["Романтические поездки","Медовый месяц, годовщина или путешествие только для двоих."],["Семейный отдых","Удобный ритм, подходящие отели и впечатления для путешественников разных возрастов."]],
    serviceEyebrow: "Без лишних забот", serviceTitle: "Всё необходимое уже включено в организацию.",
    service: [["24/7","Поддержка во время путешествия"],["120+","Проверенных отелей и партнёров"],["1","Личный travel-дизайнер"],["100%","Маршрут под ваши пожелания"]],
  ...
  en: {
    ...
    cards: [["The Amalfi Coast","Italy · 8 days","from $3,900"],["Quiet luxury in Bali","Indonesia · 10 days","from $2,800"],["Japan in blossom","Tokyo — Kyoto · 12 days","from $5,200"],["The Greek Islands","Cyclades · 9 days","from $3,400"],["Wild Iceland","Reykjavík — Vík · 7 days","from $4,100"],["Safari and ocean","Kenya — Zanzibar · 11 days","from $6,300"]],
    stylesEyebrow: "Travel your way", stylesTitle: "What kind of journey feels right for you?",
    styles: [["Beaches and islands","Private villas, clear water and unhurried days by the ocean."],["Culinary journeys","Remarkable restaurants, vineyards, markets and encounters with local chefs."],["Adventure and nature","Safaris, volcanoes, hiking and landscapes that become the main event."],["Culture and cities","Architecture, art and private access beyond the familiar tourist route."],["Romantic escapes","A honeymoon, an anniversary or a journey designed for two."],["Family journeys","A comfortable pace, thoughtful stays and experiences for every generation."]],
    serviceEyebrow: "Effortless by design", serviceTitle: "Everything you need is part of the journey.",
...
export default async function TravelDemo({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const images = ["/images/stk-lab/travel/velaria/amalfi-coast.webp", "/images/stk-lab/travel/velaria/bali-quiet-luxury.webp", "/images/stk-lab/travel/velaria/japan-sakura.webp", "/images/stk-lab/travel/velaria/greek-islands.webp", "/images/stk-lab/travel/velaria/wild-iceland.webp", "/images/stk-lab/travel/velaria/kenya-zanzibar.webp"];
  return <main className="min-h-screen bg-[#f4f0e8] text-[#243127]">
    ...
    <section className="border-y border-black/10 bg-[#e8dfcf]"><div className="mx-auto max-w-7xl px-5 py-24 md:px-8 md:py-32"><p className="text-xs uppercase tracking-[.28em] text-[#8d744f]">{t.stylesEyebrow}</p><h2 className="mt-5 max-w-4xl font-serif text-5xl tracking-[-.04em] md:text-7xl">{t.stylesTitle}</h2><div className="mt-14 grid gap-px overflow-hidden rounded-[2rem] bg-black/10 sm:grid-cols-2 lg:grid-cols-3">{t.styles.map(([title,text],i)=><article key={title} className="min-h-64 bg-[#f4f0e8] p-7 md:p-9"><span className="text-xs text-black/35">{String(i+1).padStart(2,"0")}</span><h3 className="mt-12 font-serif text-3xl">{title}</h3><p className="mt-4 leading-7 text-black/55">{text}</p></article>)}</div></div></section>

    <section className="bg-[#d8b57b] text-[#17231b]"><div className="mx-auto max-w-7xl px-5 py-20 md:px-8 md:py-24"><p className="text-xs uppercase tracking-[.28em] text-black/50">{t.serviceEyebrow}</p><h2 className="mt-5 max-w-4xl font-serif text-4xl tracking-[-.04em] md:text-6xl">{t.serviceTitle}</h2><div className="mt-14 grid grid-cols-2 gap-px overflow-hidden rounded-[2rem] bg-black/15 lg:grid-cols-4">{t.service.map(([number,label])=><div key={label} className="bg-[#d8b57b] p-6 md:p-8"><div className="font-serif text-4xl md:text-6xl">{number}</div><p className="mt-3 max-w-44 text-sm leading-6 text-black/60">{label}</p></div>)}</div></div></section>
    ...
    <section id="journeys" className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-28 md:grid-cols-2 md:px-8 md:py-36"><div className="relative aspect-[4/3] overflow-hidden rounded-[2rem]"><Image src="/images/stk-lab/travel/velaria/velaria-concierge.webp" alt="Velaria digital concierge" fill className="object-cover" sizes="(max-width:768px) 95vw, 50vw"/></div><div><p className="text-xs uppercase tracking-[.28em] text-[#8d744f]">{t.appEyebrow}</p><h2 className="mt-5 font-serif text-5xl tracking-[-.04em] md:text-7xl">{t.appTitle}</h2><p className="mt-7 text-lg leading-8 text-black/55">{t.appText}</p></div></section>
    ...
    <section id="contact" className="bg-[#17231b] px-5 py-28 text-white md:py-36"><div className="mx-auto max-w-5xl text-center"><h2 className="font-serif text-5xl tracking-[-.04em] md:text-8xl">{t.cta}</h2><p className="mx-auto mt-7 max-w-2xl text-lg leading-8 text-white/60">{t.ctaText}</p><DemoSiteOrderForm locale={locale} siteName="Velaria Travel" kind="travel" /></div></section>
