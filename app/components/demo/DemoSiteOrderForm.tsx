...
export default function DemoSiteOrderForm({ locale, siteName }: { locale: "ru" | "en"; siteName: string }) {
  ...
  return <form onSubmit={submit} className="mx-auto mt-10 max-w-3xl rounded-[2rem] border border-white/15 bg-white/[.06] p-5 text-left md:p-8"><div className="grid gap-4 md:grid-cols-2"><input name="name" required placeholder={en ? "Your name" : "Ваше имя"} className={input}/><input name="contact" required placeholder={en ? "Email or phone" : "Email или телефон"} className={input}/></div><input name="subject" placeholder={restaurant ? (en ? "Date, time and number of guests" : "Дата, время и количество гостей") : (en ? "Where would you like to travel?" : "Куда хотите отправиться?")} className={`${input} mt-4`}/><textarea name="message" rows={4} placeholder={restaurant ? (en ? "Allergies, dietary restrictions and special wishes" : "Аллергии, ограничения в питании и особые пожелания") : (en ? "Dates, travelers and wishes" : "Даты, количество путешественников и пожелания")} className={`${input} mt-4 resize-y`}/>{error&&<p className="mt-4 text-sm text-red-200">{error}</p>}<button disabled={saving} className="mt-5 w-full rounded-full bg-[#d8b57b] px-7 py-4 font-semibold text-[#17231b] disabled:opacity-60">{saving ? (en ? "Sending…" : "Отправляем…") : restaurant ? (en ? "Send reservation request" : "Отправить заявку на бронь") : (en ? "Send travel request" : "Отправить заявку на путешествие")}</button></form>;
}
