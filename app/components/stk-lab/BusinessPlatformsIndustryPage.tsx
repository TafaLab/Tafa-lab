...
const copy = {
  en: {
    ...
    portfolioText:"Business systems are highly specific to each company. Completed relevant cases will be linked here; until then, placeholders clearly show what is being prepared.",
    portfolio:[
      ["Multi-Branch Operations Platform","Sales, inventory, staff, payroll and reporting for a multi-location business.","/images/stk-lab/business-platforms/business-platform-hero.webp","Coming soon",""],
      ["Inventory & Purchasing System","Warehouses, suppliers, purchase orders and branch stock transfers.","/images/stk-lab/business-platforms/business-inventory-purchasing.webp","Coming soon",""],
      ["Workforce & Payroll Platform","Scheduling, attendance, timesheets and payroll in one system.","/images/stk-lab/business-platforms/business-staff-payroll.webp","Coming soon",""],
    ],
  ...
  ru: {
    ...
    portfolioText:"Внутренние системы сильно зависят от конкретного бизнеса. Готовые релевантные кейсы будем добавлять сюда с живыми ссылками, а пока оставляем честные заглушки.",
    portfolio:[
      ["Multi-Branch Operations Platform","Продажи, склад, сотрудники, зарплаты и отчётность для бизнеса с несколькими филиалами.","/images/stk-lab/business-platforms/business-platform-hero.webp","Проект готовится",""],
      ["Inventory & Purchasing System","Склады, поставщики, закупки и перемещения товара между филиалами.","/images/stk-lab/business-platforms/business-inventory-purchasing.webp","Проект готовится",""],
      ["Workforce & Payroll Platform","Расписание, посещаемость, табели и зарплаты в одной системе.","/images/stk-lab/business-platforms/business-staff-payroll.webp","Проект готовится",""],
    ],
...
export default function BusinessPlatformsIndustryPage({locale}:{locale:Locale}) {
  ...
  return <StkSiteShell>
    ...
    <section id="portfolio" className="bg-[var(--stk-surface-strong)] py-24 md:py-32"><div className="mx-auto max-w-7xl px-5 md:px-8"><h2 className="text-4xl tracking-[-.045em] md:text-6xl">{t.portfolioTitle}</h2><p className="mt-6 max-w-3xl text-lg leading-8 text-[color:var(--stk-muted)]">{t.portfolioText}</p><div className="mt-14 grid gap-5 md:grid-cols-3">{t.portfolio.map(([name,desc,img,status,url])=>{const card=<div className="group h-full overflow-hidden rounded-[var(--stk-radius-card)] border border-[color:var(--stk-border)] bg-[color:var(--stk-surface-card)]"><div className="relative aspect-[16/10] overflow-hidden"><Image src={img} alt={name} fill className="object-cover transition duration-700 group-hover:scale-[1.03]" sizes="(max-width:768px) 95vw, 32vw"/><div className={`absolute inset-0 ${url?"":"bg-[var(--stk-dark)]/20"}`}/><span className="absolute bottom-5 left-5 rounded-full bg-white/90 px-3 py-1.5 text-xs">{status}</span></div><div className="p-6"><h3 className="text-2xl tracking-[-.04em]">{name}</h3><p className="mt-3 leading-7 text-[color:var(--stk-muted)]">{desc}</p>{url&&<div className="mt-6 text-sm font-medium">{locale==="ru"?"Открыть проект":"View live project"} →</div>}</div></div>;return url?<a key={name} href={url} target="_blank" rel="noreferrer">{card}</a>:<div key={name}>{card}</div>})}</div></div></section>
