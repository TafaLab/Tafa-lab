...
export default function BakeryIndustryPage({ locale }: { locale: StkLabLocale }) {
  ...
  return (
    <StkSiteShell>
      ...
      <section className="border-y border-[color:var(--stk-border)] bg-[var(--stk-surface-strong)] py-24 md:py-32"><div className="mx-auto grid max-w-7xl gap-12 px-5 md:grid-cols-[.75fr_1.25fr] md:px-8"><h2 className="text-4xl tracking-[-.045em] md:text-6xl">{t.faqTitle}</h2><div className="divide-y divide-black/10 border-y border-[color:var(--stk-border)]">{t.faqs.map(([q,a],i)=><details key={q} className="group py-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-xl tracking-[-.025em]"><span>{q}</span><span className="text-[color:var(--stk-faint)] transition group-open:rotate-45">+</span></summary><p className="max-w-2xl pb-2 pt-5 leading-7 text-[color:var(--stk-muted)]">{a}</p></details>)}</div></div></section>
