...
const copy = {
  en: {
    ...
    services:[
      ...
      ["Bookings & Operations","Custom","Hotels, transfers, tours, supplier confirmations, costs, margins and operational checklists."],
      ["Digital Concierge","Custom","Mobile traveler experience with today's plan, documents, alerts, transfers and concierge access."],
    ],
    builderEyebrow:"Custom Trip Builder", builderTitle:"Let clients shape the journey before they send the request.",
    builderText:"A guided trip builder collects destination, dates, travelers, hotel preferences, experiences, transfers and special requests in one premium flow.",
    itineraryEyebrow:"Interactive Itinerary", itineraryTitle:"Put the entire journey in one elegant digital space.",
    ...
    portfolio:[
      ...
      ["Velaria Travel","Premium bespoke travel agency demo with curated journeys and digital concierge.","/images/stk-lab/travel/travel-website-hero.webp","Live demo","travel-demo"],
    ...
    process:[
      ["01","Discovery","Destinations, audience, products, sales process and operational needs."],
      ["02","Journey Design","From inspiration and enquiry to itinerary, payment and travel."],
      ["03","Visual Direction","Premium editorial design tailored to the travel brand."],
      ["04","Development","Responsive build, custom flows, integrations and internal systems."],
      ["05","Launch & Growth","Testing, launch and expansion into CRM, operations or concierge tools."],
    ],
    ...
    faqTitle:"FAQ",
    faqs:[
      ["Can the website collect custom trip requests?","Yes. We can create structured enquiry or trip-builder flows around the way your company plans journeys."],
      ["Can clients receive a digital itinerary?","Yes. It can include daily plans, hotels, restaurants, transfers, activities, reservations and important documents."],
      ["Can the system manage supplier bookings and payments?","Yes. Internal tools can track suppliers, confirmations, costs, client payments, outstanding balances and margins."],
      ["Can we start with a website and add CRM later?","Yes. The public website can launch first and the internal travel platform can be expanded later."],
    ],
    contactEyebrow:"Start a project", contactTitle:"Ready to create a stronger digital journey for your travel business?",
    contactText:"Tell us how you sell trips today and which parts of the client or operations journey you want to improve.", contactCta:"Discuss on WhatsApp",
  },
  ru: {
    ...
    services:[
      ...
      ["Бронирования и операции","Индивидуально","Отели, трансферы, экскурсии, подтверждения поставщиков, расходы, маржа и чек-листы."],
      ["Digital Concierge","Индивидуально","Мобильный кабинет путешественника: план дня, документы, уведомления, трансферы и связь с менеджером."],
    ],
    builderEyebrow:"Конструктор путешествия", builderTitle:"Клиент формирует поездку ещё до отправки заявки.",
    builderText:"Пошаговый конструктор собирает направление, даты, количество путешественников, отель, впечатления, трансферы и особые пожелания в одном премиальном сценарии.",
    itineraryEyebrow:"Интерактивный маршрут", itineraryTitle:"Всё путешествие — в одном красивом цифровом пространстве.",
    ...
    portfolio:[
      ...
      ["Velaria Travel","Премиальный демо-сайт агентства индивидуальных путешествий с маршрутами и digital concierge.","/images/stk-lab/travel/travel-website-hero.webp","Открыть демо","travel-demo"],
    ...
    process:[
      ["01","Исследование","Направления, аудитория, продукты, продажи и операционные задачи."],
      ["02","Путь клиента","От вдохновения и заявки до маршрута, оплаты и самой поездки."],
      ["03","Визуальная система","Премиальный editorial-дизайн под характер travel-бренда."],
      ["04","Разработка","Адаптивный сайт, индивидуальные сценарии, интеграции и внутренние системы."],
      ["05","Запуск и развитие","Тестирование, запуск и расширение до CRM, операций и concierge-инструментов."],
    ],
    ...
    faqs:[
      ["Можно собирать индивидуальные заявки на путешествия?","Да. Можно сделать обычную структурированную заявку или полноценный trip builder под процесс вашей компании."],
      ["Можно выдавать клиенту цифровой маршрут?","Да. В нём могут быть планы по дням, отели, рестораны, трансферы, активности, бронирования и документы."],
      ["Можно управлять бронированиями поставщиков и оплатами?","Да. Внутренняя система может учитывать поставщиков, подтверждения, себестоимость, оплаты клиента, остатки и маржу."],
      ["Можно сначала сделать сайт, а CRM добавить позже?","Да. Публичный сайт можно запустить первым, а внутреннюю travel-платформу расширять позже."],
    ],
    contactEyebrow:"Начать проект", contactTitle:"Хотите создать более сильный цифровой путь для travel-бизнеса?",
    contactText:"Расскажите, как вы сейчас продаёте путешествия и какие этапы работы с клиентом или операциями хотите улучшить.", contactCta:"Обсудить в WhatsApp",
  }
} as const;

export default function TravelIndustryPage({locale}:{locale:Locale}) {
  const t=copy[locale], other=locale==="ru"?"en":"ru";
  const message=encodeURIComponent(locale==="ru"?"Здравствуйте! Хочу обсудить сайт или цифровую систему для travel-бизнеса.":"Hello! I'd like to discuss a website or digital system for a travel business.");
  return <StkSiteShell>
    ...
    <section id="portfolio" className="bg-[var(--stk-surface-strong)] py-24 md:py-32"><div className="mx-auto max-w-7xl px-5 md:px-8"><h2 className="text-4xl tracking-[-.045em] md:text-6xl">{t.portfolioTitle}</h2><p className="mt-6 max-w-3xl text-lg leading-8 text-[color:var(--stk-muted)]">{t.portfolioText}</p><div className="mt-14 grid gap-5 md:grid-cols-2">{t.portfolio.map(([name,desc,img,status,url],i)=>{const href=url==="travel-demo"?`/${locale}/travel-demo`:url;const card=<div className="group h-full overflow-hidden rounded-[var(--stk-radius-card)] border border-[color:var(--stk-border)] bg-[color:var(--stk-surface-card)]"><div className="relative aspect-[16/10] overflow-hidden"><Image src={img} alt={name} fill className="object-cover transition duration-700 group-hover:scale-[1.03]" sizes="(max-width:768px) 95vw, 48vw"/><div className={`absolute inset-0 ${href?"":"bg-[var(--stk-dark)]/20"}`}/><span className="absolute bottom-5 left-5 rounded-full bg-white/90 px-3 py-1.5 text-xs">{status}</span></div><div className="p-6 md:p-8"><h3 className="text-3xl tracking-[-.04em]">{name}</h3><p className="mt-3 leading-7 text-[color:var(--stk-muted)]">{desc}</p>{href&&<div className="mt-6 text-sm font-medium">{locale==="ru"?"Открыть проект":"View live project"} →</div>}</div></div>;return href?<a key={name} href={href} target="_blank" rel="noreferrer" className={i===0?"md:col-span-2":""}>{card}</a>:<div key={name}>{card}</div>})}</div></div></section>
