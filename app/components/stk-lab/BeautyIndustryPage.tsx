...
const copy = {
  en: {
    ...
    portfolioTitle:"Beauty product concepts",
    portfolioText:"These visuals demonstrate the types of beauty products and systems STK Lab can build. Live beauty case studies will be added here as we create them.",
    portfolio:[
      ["Beauty Website","Premium salon website experience.","/images/stk-lab/beauty/beauty-website-hero.webp","Product concept"],
      ["Online Booking","Branded appointment booking flow.","/images/stk-lab/beauty/beauty-online-booking.webp","Product concept"],
      ["Services Catalogue","Digital treatments and pricing catalogue.","/images/stk-lab/beauty/beauty-services-catalogue.webp","Product concept"],
      ["Beauty CRM","Salon management and client dashboard.","/images/stk-lab/beauty/beauty-crm-dashboard.webp","Business system"],
      ["Staff Schedule","Team calendar and appointment workload.","/images/stk-lab/beauty/beauty-staff-schedule.webp","Business system"],
      ["Owner Mobile","Mobile business analytics and alerts.","/images/stk-lab/beauty/beauty-owner-mobile.webp","Business system"],
    ],
  ...
  ru: {
    ...
    portfolioTitle:"Концепты beauty-продуктов",
    portfolioText:"Эти визуалы показывают, какие сайты и системы STK Lab может создавать для beauty-бизнеса. Живые beauty-кейсы будем добавлять сюда по мере их создания.",
    portfolio:[
      ["Beauty Website","Премиальный сайт салона.","/images/stk-lab/beauty/beauty-website-hero.webp","Концепт продукта"],
      ["Онлайн-запись","Фирменный сценарий записи клиента.","/images/stk-lab/beauty/beauty-online-booking.webp","Концепт продукта"],
      ["Каталог услуг","Цифровой каталог процедур и цен.","/images/stk-lab/beauty/beauty-services-catalogue.webp","Концепт продукта"],
      ["Beauty CRM","Управление салоном и клиентами.","/images/stk-lab/beauty/beauty-crm-dashboard.webp","Бизнес-система"],
      ["Расписание мастеров","Календарь команды и загрузка записей.","/images/stk-lab/beauty/beauty-staff-schedule.webp","Бизнес-система"],
      ["Кабинет владельца","Мобильная аналитика и уведомления.","/images/stk-lab/beauty/beauty-owner-mobile.webp","Бизнес-система"],
    ],
...
export default function BeautyIndustryPage({locale}:{locale:Locale}) {
  ...
  return <StkSiteShell>
    ...
    <section id="portfolio" className="bg-[var(--stk-surface-strong)] py-24 md:py-32"><div className="mx-auto max-w-7xl px-5 md:px-8"><h2 className="text-4xl tracking-[-.045em] md:text-6xl">{t.portfolioTitle}</h2><p className="mt-6 max-w-3xl text-lg leading-8 text-[color:var(--stk-muted)]">{t.portfolioText}</p><div className="mt-14 grid gap-5 md:grid-cols-2">{t.portfolio.map(([name,desc,img,status])=><div key={name} className="group overflow-hidden rounded-[var(--stk-radius-card)] border border-[color:var(--stk-border)] bg-[color:var(--stk-surface-card)]"><div className="relative aspect-[16/10] overflow-hidden"><Image src={img} alt={name} fill className="object-cover transition duration-700 group-hover:scale-[1.03]" sizes="(max-width:768px) 95vw, 48vw"/><span className="absolute bottom-5 left-5 rounded-full bg-white/90 px-3 py-1.5 text-xs">{status}</span></div><div className="p-6 md:p-8"><h3 className="text-3xl tracking-[-.04em]">{name}</h3><p className="mt-3 leading-7 text-[color:var(--stk-muted)]">{desc}</p></div></div>)}</div></div></section>
