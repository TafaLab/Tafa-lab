...
const copy = {
  en: {
    ...
    intro2:"Tafa Lab combines brand-led web design with practical restaurant technology, from digital menus and table reservations to owner dashboards and operational systems.",
  ...
  ru: {
    ...
    intro2:"Tafa Lab объединяет брендовый веб-дизайн с практичными ресторанными технологиями: от цифрового меню и бронирования до дашбордов владельца и операционных систем.",
...
export default function RestaurantIndustryPage({locale}:{locale:Locale}) {
  ...
  return <StkSiteShell>
    <header className="sticky top-0 z-50 border-b border-[color:var(--stk-border)] bg-[color:var(--stk-bg-translucent)] backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8"><Link href={`/${locale}`} aria-label="Tafa Lab"><TafaLabLogo priority /></Link><nav className="hidden items-center gap-7 text-sm md:flex"><a href="#services">{t.navServices}</a><a href="#portfolio">{t.navPortfolio}</a><a href="#process">{t.navProcess}</a><a href="#contact">{t.navContact}</a></nav><Link href={`/${other}/industries/restaurants`} className="rounded-full border border-[color:var(--stk-border)] px-3 py-2 text-xs uppercase">{other}</Link></div></header>
